...
def rgb2ycbcr(rgb_img):
    ...
    return ycbcr_img.astype(np.float32)
...
def paired_random_crop(img_gts, img_lqs, gt_patch_size, scale=1):
    """Paired random crop.
...
def totensor(imgs, opt_bgr2rgb=True, float32=True):
    ...
    def _totensor(img, opt_bgr2rgb, float32):
        if img.shape[2] == 3 and opt_bgr2rgb:
            img = bgr2rgb(img)   # (H W [RGB])
        img = torch.from_numpy(img.transpose(2, 0, 1))  # ([RGB] H W)
        if float32:
            img = img.float()
        return img
